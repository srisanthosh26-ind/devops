    ...
    stages {
        ...
        stage('Push Docker Image to DockerHub') {
            steps {
               ...
               withCredentials([string(credentialsId: 'dockerhubpwd',usernameVariable: 'DOCKER_USER', passwordVariable: 'DOCKER_PASS')]) {
                    bat '''
                        echo %DOCKER_PASS% | docker login -u %DOCKER_USER% --password-stdin
                        docker tag my-pipeline-app:v1 %DOCKER_USER%/my-pipeline-app:v1
                        docker push %DOCKER_USER%/my-pipeline-app:v1
                    '''
               }
            }
        }
        ...
        stage('Deploy the project using Container') {
            steps {
                ...
                bat '''
                    ...
                    docker run --name myjavaappcont %DOCKER_USER%/my-pipeline-app:v1
                '''
            }
        }
    }
